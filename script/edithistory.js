                        ...
                        for (let i = 0; i < Object.keys(resJson).length; i++) {
                            if(resJson[i]['id'] == id) {
                                if(resJson[i]['user_id'] == localStorage.getItem('u_id')) {
                                    ...
                                    date.innerHTML = htmlEscape(resJson[i]['date']);
                                } else {
                            ...
                            if(i === Object.keys(resJson).length - 1) {
                                formAllDisabled();
                                document.getElementById('msg').textContent = '対象の履歴が見つかりませんでした。';
                            }
                        }
                    }
                }
...
document.getElementById('edit').onsubmit = function(event) {
    ...
    if(form.datetime.value !== '') {
        editobj.date = form.datetime.value;
    }
...
function putUpdate(obj) {
    ...
    request.open('put', BASE_URL + '/history/update/');
    console.log('送信先: ' + BASE_URL + '/history/update/');
